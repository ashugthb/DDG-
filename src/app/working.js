...
// Generate initial channel data for each brain
function generateBrainChannels(brainId) {
  const channels = [];
...
function drawAdvancedContourMap(ctx, channels, width, height, colors, outlineImg) {
  ...
  const brainY = centerY - brainHeight / 2;
...
export default function MultiBrainVisualization() {
  ...
  const initializeBrains = useCallback(() => {
    // Create 12 brains with different activity patterns
    return Array.from({ length: 12 }, (_, i) => ({
      id: i,
      channels: generateBrainChannels(i),
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || brains.length === 0) return;
        ...
        <AppContainer ref={containerRef} sx={{ position: 'relative' }}>
          <AppBar position="static" elevation={0} color="transparent">
            ...
            <Box sx={{ 
              ...
              <Typography variant="body1">Real-time Visualization of 12 Brain Activity Patterns</Typography>
          ...
          <Container maxWidth={false} sx={{ flexGrow: 1, py: 1, position: 'relative' }}>
            <Grid container spacing={1.5} sx={{ height: '100%' }}>
              {brains.map(brain => (
                <Grid item xs={3} key={brain.id} sx={{ height: '33%' }}>
                  <BrainCell elevation={4}>
                    <BrainVisualization 
                      brainId={brain.id}
